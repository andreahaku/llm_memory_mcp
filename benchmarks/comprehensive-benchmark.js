...
exports.ComprehensiveBenchmarkRunner = void 0;
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
const perf_hooks_1 = require("perf_hooks");
const QRManager_js_1 = require("../src/qr/QRManager.js");
const QRDecoder_js_1 = require("../src/qr/QRDecoder.js");
const NativeEncoder_js_1 = require("../src/video/NativeEncoder.js");
const WasmEncoder_js_1 = require("../src/video/WasmEncoder.js");
const FrameIndex_js_1 = require("../src/video/FrameIndex.js");
const VideoEncoder_js_1 = require("../src/video/VideoEncoder.js");
 ...
 */
function generateContentSamples() {
    return [
        {
            name: 'small-snippet',
            type: 'code-snippet',
...
class QRBenchmarkSuite {
    qrManager = new QRManager_js_1.QRManager();
    qrDecoder = new QRDecoder_js_1.QRDecoder();
    results = [];
    async runQREncodingBenchmarks(samples, config) {
    ...
    async benchmarkQREncoding(sample, config) {
        ...
        for (let i = 0; i < config.iterations; i++) {
            ...
            try {
                ...
                totalMemoryPeak += Math.max(memoryAfter.heapUsed - memoryBefore.heapUsed, memoryAfter.external - memoryBefore.external);
            }
            catch (error) {
    ...
    async benchmarkQRDecoding(sample, config) {
...
class VideoEncodingBenchmarkSuite {
    results = [];
    async runVideoEncodingBenchmarks(samples, config) {
        ...
        if (config.testBothEncoders) {
            encoders.push({
                name: 'Native FFmpeg',
                create: async () => {
                    if (await (0, NativeEncoder_js_1.isNativeEncoderSupported)()) {
                        const encoder = new NativeEncoder_js_1.NativeFFmpegEncoder();
                ...
                name: 'WASM FFmpeg',
                create: async () => {
                    if (await (0, WasmEncoder_js_1.isWasmEncoderSupported)()) {
                        const encoder = new WasmEncoder_js_1.WasmFFmpegEncoder();
                        await encoder.initialize();
                        return encoder;
                    }
                    return null;
            ...
            encoders.push({
                ...
                create: async () => {
                    ...
                    else if (await (0, WasmEncoder_js_1.isWasmEncoderSupported)()) {
                        const encoder = new WasmEncoder_js_1.WasmFFmpegEncoder();
                        await encoder.initialize();
                        return encoder;
                    }
    ...
    async benchmarkVideoEncoding(encoder, sample, encoderName, config) {
        console.log(`📊 Video Encoding (${encoderName}): ${sample.name}`);
        // Generate QR frames for the sample
        const qrManager = new QRManager_js_1.QRManager();
        ...
        const profiles = [
            { name: 'high-quality', options: VideoEncoder_js_1.QR_ENCODING_PROFILES.HIGH_QUALITY_FAST },
            { name: 'balanced', options: VideoEncoder_js_1.QR_ENCODING_PROFILES.BALANCED },
            { name: 'compact', options: VideoEncoder_js_1.QR_ENCODING_PROFILES.COMPACT }
        ];
...
class EndToEndBenchmarkSuite {
    results = [];
    async runEndToEndBenchmarks(samples, config) {
        console.log('🔄 Running End-to-End Pipeline Benchmarks...\n');
        for (const sample of samples) {
            await this.benchmarkFullPipeline(sample, config);
        }
    }
    async benchmarkFullPipeline(sample, config) {
        console.log(`📊 End-to-End Pipeline: ${sample.name}`);
        const qrManager = new QRManager_js_1.QRManager();
        const qrDecoder = new QRDecoder_js_1.QRDecoder();
        // Use best available encoder
        let encoder = null;
        if (await (0, NativeEncoder_js_1.isNativeEncoderSupported)()) {
            encoder = new NativeEncoder_js_1.NativeFFmpegEncoder();
        }
        else if (await (0, WasmEncoder_js_1.isWasmEncoderSupported)()) {
            encoder = new WasmEncoder_js_1.WasmFFmpegEncoder();
        }
        if (!encoder) {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            try {
                ...
                const videoResult = await encoder.encode(qrResult.frames, VideoEncoder_js_1.DEFAULT_QR_ENCODING_OPTIONS);
                // Step 3: Video → Frame index (simulate .mvi file)
                const mviData = (0, FrameIndex_js_1.createMviFile)(videoResult.frameIndex);
                (0, FrameIndex_js_1.readMviFile)(mviData); // Validate MVI file creation
...
class FrameSeekingBenchmarkSuite {
    results = [];
    async runFrameSeekingBenchmarks(config) {
    ...
    async benchmarkFrameSeeking(testCase, config) {
        ...
        // Create MVI file
        const mviData = (0, FrameIndex_js_1.createMviFile)(frameIndex);
        const mviReader = new FrameIndex_js_1.MviReader(mviData);
        const seekLatencies = [];
        const randomFrameNumbers = Array.from({ length: config.iterations * 10 }, () => Math.floor(Math.random() * testCase.frames));
...
class ComprehensiveBenchmarkRunner {
    ...
    async generateReport(totalExecutionTime) {
    ...
    generateRecommendations() {
        ...
        const avgCompression = Object.values(summary).reduce((sum, cat) => sum + (cat.avgCompressionRatio || 0), 0) / Object.keys(summary).length;
        if (avgCompression < 2) {
...
exports.ComprehensiveBenchmarkRunner = ComprehensiveBenchmarkRunner;
// =============================================================================
// CLI ENTRY POINT
// =============================================================================
async function main() {
...
//# sourceMappingURL=comprehensive-benchmark.js.map